...
ArrayView<Scalar> ArrayNonConstBase<Derived, Scalar>::operator() (T1 m, T2 n, T3 o, T4 p, T5 q, OtherTypes... otherSpans)
{
	MDArrayView<Scalar> domiView = getMdArrayView();

	constexpr int nDims = 5 + sizeof...(OtherTypes);
#ifdef FTN_DEBUG
	if (nDims != numDims())
	{
		std::ostringstream strStream;
	...
	sliceDomiView(domiView, o, 3);
	sliceDomiView(domiView, p, 4);
	sliceDomiView(domiView, q, 5);
	DimObject dimObjects[nDims-5] = {DimObject(&otherSpans)...};
	for (int dimNum = 6, i = 0; dimNum <= nDims; dimNum++, i++)
	{
		if (dimObjects[i].activeMember == 0)
		{
			sliceDomiView(domiView, *dimObjects[i].dU.index, dimNum);
		}
		else
		{
			sliceDomiView(domiView, *dimObjects[i].dU.sp, dimNum);
		}
	}
...
template<class Derived, class Scalar>
template<class T1, class T2, class T3, class T4, class T5, class... OtherTypes>
ArrayView<Scalar> ArrayNonConstBase<Derived, Scalar>::operator() (T1 m, T2 n, T3 o, T4 p, T5 q, OtherTypes... otherSpans) const
{
	MDArrayView<Scalar> domiView = getMdArrayView();
...
template<class Derived, class Scalar>
void ArrayNonConstBase<Derived, Scalar>::sliceDomiView(MDArrayView<Scalar>& domiView, span& indexSpan, int dimNum) const
{
	dim_type startInd = indexSpan.start();
	dim_type stopInd = indexSpan.stop();
	dim_type step = indexSpan.step();

	if (startInd == Dynamic)
	{
		startInd = lbound(dimNum);
	}
	if (stopInd == Dynamic)
	{
		stopInd = ubound(dimNum);
	}

#ifdef FTN_DEBUG
	indexOutOfBounds(startInd, dimNum);
	indexOutOfBounds(stopInd, dimNum);
#endif

	startInd -= lbound(dimNum);
	stopInd -= lbound(dimNum);
	stopInd += step;

	domiView = std::move(domiView[span(startInd, stopInd, step)]);
}

template<class Derived, class Scalar>
void ArrayNonConstBase<Derived, Scalar>::sliceDomiView(MDArrayView<Scalar>& domiView, dim_type index, int dimNum) const
{
#ifdef FTN_DEBUG
	indexOutOfBounds(index, dimNum);
#endif
	index -= lbound(dimNum);
	domiView = std::move(domiView[index]);
}
